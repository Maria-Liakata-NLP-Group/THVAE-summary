...
class SumSampler(BaseChunkCollector):
    ...
    def __init__(self, id_fname, min_units=None, max_units=None,
        ...
        :param sample_all: if set to True performs sampling of reviews until
                                ...
                                drops the rest.
        """
        super(SumSampler, self).__init__(max_size=1)
        ...
        if self.max_units:
            while len(self):
                if len(self) > self.max_units:
                    ...
                    #                           size=self.max_units)
                    sel_indxs = range(self.max_units)
                else:
                    sel_indxs = range(len(self))

                # create an output data-chunk based on the selected units
                dc = DataChunk()
                # group_id ['309198_84', '309198_84', '309198_84', '309198_84', '309198_84', '309198_84', '309198_84', '309198_84', '309198_84', '309198_84']
                # this is the relationship between k and val
                for k, val in self._coll.items():
                    # dc[k] = [val[indx] for indx in sel_indxs]
                    # if isinstance(val, np.ndarray):
                    #     dc[k] = np.array(dc[k], dtype=val.dtype)
                    dc[k] = [val[indx] for indx in sel_indxs]
                    if len(sel_indxs) < self.max_units:
                        for i in range((self.max_units - len(sel_indxs))):
                            dc[k].append(val[len(sel_indxs)-1])
                    if isinstance(val, np.ndarray):
                        dc[k] = np.array(dc[k], dtype=val.dtype)
                yield dc
