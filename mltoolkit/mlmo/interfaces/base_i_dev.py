...
class BaseIDev(BaseInterface):
    ...
    def __init__(self, imodel, train_data_pipeline, gen_sum_pipeline, val_data_pipeline=None,
                 name_prefix=None):
        ...
        self.val_data_pipeline = val_data_pipeline if val_data_pipeline else \
            train_data_pipeline
        self.gen_sum_pipeline = gen_sum_pipeline
    ...
    def train(self, data_source, epoch, logging_steps=10, **kwargs):
        ...
        for i, batch in enumerate(self.train_data_pipeline.iter(**data_source),
                                  1):
            metrics = self.imodel.train(batch=batch, epoch=epoch, **kwargs)
