...
class GruPointerDecoder(Module):
    ...
    def __init__(self, input_dim, contxt_dim, hidden_dim, att_module,
                 pointer_gen_module,transformer_decoder, word_padding_idx=0, cat_contx_to_inp=True,
    ...
    def forward(self, embds, mask, init_hidden, att_keys=None, att_values=None,
                att_word_ids=None, att_mask=None, init_cont=None, extra_feat=None, review_word_id=None,
                **att_kwargs):
        """
        ...
        # decode_output = self.decoder.layers[3](hidden_states=extra_feat)
        # bart_hidden = decode_output[0]

        decode_output = self.decoder(inputs_embeds=extra_feat)
        ...
        for indx in range(out_seq_len):
            ...
            src_features = bart_hidden[:, index, :]
            ...
            hidden, cont, att_wts = self.step(_x, _m,
    ...
    def step(self, emb, m, prev_hidden, prev_cont, att_keys, att_values,
             att_mask, src=None, src_features=None, mode=None, **att_kwargs):
        """
        ...
        # decode_output = self.decoder(inputs_embeds=src_features)

        # decode_output = self.decoder.layers[3](hidden_states=src_features)
        # hidden = decode_output[0]

        # hidden = decode_output.last_hidden_state
        # etr_inp = hidden[:, -1, :]

        etr_inp = src_features
