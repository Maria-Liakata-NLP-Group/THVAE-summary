...
class IDevCopyCat(BaseIDev):
    ...
    def train(self, data_source, epoch, logging_steps=10, **kwargs):
        """
        ...
        for indx, batch in enumerate(data_chunk_iter, 1):
            ...
            metrics = self.imodel.train(batch, epoch=epoch, c_lambd=c_lambd, z_lambd=z_lambd)
    ...
    def gen_and_save_summs(self, data_source, output_file_path):
        ...
        # for i, dc in enumerate(self.gen_sum_pipeline.iter(**data_source), 1):
        for i, dc in enumerate(self.val_data_pipeline.iter(**data_source), 1):
            # group_id = dc[ModelF.GROUP_ID]
            # print(group_id)
            _, gen_revs, _, gen_summ, collect_z = self.imodel.predict(dc)
            # print(gen_summ)
            # sum1 = gen_summ[0]
            # sum2 = gen_summ[1]
            # gen_summ.append(sum1)
            # gen_summ.append(sum2)
            # sum2 = gen_summ[1]
            # sum3 = gen_summ[2]
            # gen_summ[1] = sum1
            # gen_summ[2] = sum2
            # gen_summ.append(sum2)
            # gen_summ.append(sum3)
            # gen_summ.append(sum3)
            ...
            new_dc[ModelF.GEN_Z1] = collect_z[0]
            new_dc[ModelF.GEN_Z2] = collect_z[1]
            new_dc[ModelF.GEN_Z3] = collect_z[2]
            new_dc[ModelF.GEN_Z4] = collect_z[3]
            new_dc[ModelF.GEN_SUMM] = gen_summ

            # seq_fnames = [ModelF.GEN_SUMM, ModelF.GEN_REV, ModelF.REV]
            seq_fnames = [ModelF.GEN_SUMM, ModelF.GEN_REV, ModelF.REV, ModelF.GEN_Z1, ModelF.GEN_Z2, ModelF.GEN_Z3, ModelF.GEN_Z4]
            ...
            for fn in seq_fnames:
                out_str = self.imodel.model.tokenizer.batch_decode(new_dc[fn], skip_special_tokens=True)
                new_dc[fn] = np.array(out_str)
            ...
            for fn in [ModelF.GEN_REV, ModelF.REV, ModelF.GEN_Z1, ModelF.GEN_Z2, ModelF.GEN_Z3, ModelF.GEN_Z4]:
    ...
    def summ_generator(self, batch, summ_post_proc=None):
        ...
        summs_word_output = self.imodel.generate_summaries(batch)

        # summs_strs, _ = self.imodel.translate_sentence(summary_output=summs_word_output, num_beams=3)
        summs_strs, _ = self.imodel.translate_sentence(outputs_id=summs_word_output)
        # summs_strs, _ = self.imodel.translate_sentence_beamsearch(summary_output=summs_word_output, num_beams=3)
