...
class CustomBartLayer(nn.Module):
    def __init__(self, z_fun, bart_layer):
        ...
        # self.seq_list = list()
        # # self.seq_list.append(self.adapter_norm_before)
        # self.adapter_pro_down = Linear(768, 384)
        # # nn.init.xavier_uniform_(self.adapter_pro_down.weight)
        # self.seq_list.append(self.adapter_pro_down)
        self.non_linearity = ELU()
        # self.adapter_down = nn.Sequential(*self.seq_list)
        ...
        # self.adapter_up = Linear(384, 768)
...
class CopyCat(Module):
    ...
    def __init__(self, arch_instance,  vocab_size, ext_vocab_size, emb_dim, enc_hidden_dim,
        ...
        # [batch_size, group_size(8), 100 (post_length), dimension_size(768)]

        #                              80
        # [batch_size (6) * group_size(8), dimension_size(768)]
        ...
        # self.conv1d = nn.Conv1d(self.enc_hidden_dim, self.num_channels_enc, 3, stride=1, padding=1)

        # self.z_atten = AverageSelfAttention(768)
    ...
    def init_stem(self):
        ...
        # Cin = self.num_channels_enc
        Cout = self.num_channels_enc
        # Cin = self.input_dim_lstm
        Cin = self.enc_hidden_dim
        # 这里可能需要多个卷积和，先这样，也可能不需要，stride为1 积累特征再说
        # kernels = [1, 2, 3, 4, 5]
        # kernel_features = self.kernel_features
        # layers = ModuleList()
        # for kernel_size, kernel_feature_size in zip(kernels, kernel_features):
        #     stem = Conv1D(Cin, kernel_feature_size, kernel_size, padding=0, bias=True)
        #     layers.append(stem)
        # return layers
    ...
    def init_conditional(self, mult):
        ...
        # C_in = int(self.num_channels_dec)
        ...
        #         C_out = 2 *3
        #     else:
        # C_out = int(self.num_channels_dec * 2)
    ...
    def forward(self, rev, rev_len, rev_mask,
        ...
        # out_put = self.bart_model.model.encoder(input_ids=rev)
        # rev_hiddens = out_put.last_hidden_state

        # out_put, p_q, first_rev_hiddens = self.get_finals_output(rev=rev)
        # rev_hiddens = out_put.encoder_last_hidden_state
        # rev_word_embds = self._embds(rev) * self.bart_model.model.encoder.embed_scale
        ...
        # decode_output = self.bart_model.model.decoder(input_ids=rev, encoder_hidden_states=z)
        # bart_hidden = decode_output.last_hidden_state
        # lm_logits = self.bart_model.lm_head(bart_hidden)
        # log_probs = lm_logits + self.bart_model.final_logits_bias.to(lm_logits.device)

        # self.bart_model.model.encoder

        # z = self.AvgPool(T.transpose(z, 2, 1))
        # z = z.squeeze(-1)

        # all_q = p_q['all_q']
        # all_p = p_q['all_p']
        # all_log_q = p_q['all_log_q']
        # all_log_p = p_q['all_log_p']

        # lm_logits = out_put.logits

        # lm_logits = self.bart_model.lm_head(decoder)
        # lm_logits = lm_logits + self.bart_model.final_logits_bias.to(lm_logits.device)


        # rev_word_embds = self._embds(rev)
        # rev_encs, rev_hiddens = self.encode(rev_word_embds, rev_len)
        # s = T.transpose(rev_hiddens, 2, 1)

        # prom_emb = self._embds(prompt)
        # prom_encs, prom_hiddens = self.encode(prom_emb, prompt_len)

        ################################################################ get z #################################################################

        # z, all_q, all_p, all_log_q, all_log_p = self.get_z(s)
        # att_keys = self.create_att_keys(first_rev_hiddens)
        ########################################################### from here, it's a normal decoder ###########################################
    ...
    def get_final_output(self, layer_num=3, rev=None, input_emb=None, summary=False):
        ...
        if summary:
            # 所有encoder的输出作为distribution，构建summary，所以有两个策略，1.它可以作为encoder的输入，然后输出再输入decoder 2. 它可以直接作为decoder的输入，
            # 注意： 只有decodre的输出才可以用于beamsearch
            ...
            # dec_out = self.bart_model.model.decoder(inputs_embeds=latent_code)
            # beam_input = dec_out.last_hidden_state
            return latent_code
            # return beam_input
        else:
            # 这个output 应该同时输出 encoder的output 和 decoder 的output（PS： encoder的output适用于构建summary的distribution，然后这个decoder的output是用于beamsearch）
            out_put= self.bart_model.model(input_ids=rev, output_hidden_states=True)
            # check = self.bart_model.model.output[0]
    ...
    # def get_z(self, s, prom_hiddens, att=True):
    def get_z(self, s):
        ...
        # prior for z0 notice，they use autogressive, so need the first z0， 看到没这个Normal的是一堆0，然后log方法后，还是z
        ...
        # prompt 从第二个开始就可以，因为有个start和end 标志, repeat key_info 为z的长度，做相似度测试，然后选出最大的那个值

        # if att:
        #     # stetegy 1
        #     # z_attn = self.get_attention(z, prom_hiddens)
        #     # attn_rev_len = T.ones(batch_size)
        #     # attn_rev_len = attn_rev_len * z_attn.size()[1]
        #     # z_attn_enc, _ = self.enc_summary(z, attn_rev_len)
        #
        #     # strategy 2
        #     z_attn_enc = self.get_attention(z, prom_hiddens)
        # else:
        #     z_attn_enc = None

        # return z_final_enc, all_q, all_p, all_log_q, all_log_p, z_attn_enc
