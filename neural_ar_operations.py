...
class ARConv1d(nn.Conv1d):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
        ...
        if self.masked:
            assert kernel_size % 2 == 1, 'kernel size should be an odd value.'
                        # 把数组转成张量，且共享内存, 对于我们的任务文本需不需mask？ 似乎需要，因为对于文本后面的padding 根本不需要被卷积
        ...
        # init weight normalizaition parameters 换成1d，这个view是不是要改一下
        # init = torch.log(norm(self.weight * init_mask, dim=[1, 2, 3]).view(-1, 1, 1, 1) + 1e-2)
        init = torch.log(norm(self.weight * init_mask, dim=[1, 2]).view(-1, 1, 1) + 1e-2)
    ...
    def forward(self, x):
        ...
        bias = self.bias
        # 对于文本来说 conv1d 就够了
        # conv2d(input, weight(filters of shape), bias(optional bias tensor of shape), stride, padding, dilation, groups)
        # return F.conv2d(x, self.weight_normalized, bias, self.stride,
        #                 self.padding, self.dilation, self.groups)
        return F.conv1d(x, self.weight_normalized, bias, self.stride, self.padding)
...
class ARInvertedResidual(nn.Module):
    def __init__(self, inz, inf, ex=6, dil=1, k=5, mirror=False):
        ...
        padding = dil * (k - 1) // 2
        layers = []
        layers.extend([ARConv1d(inz, hidden_dim, kernel_size=3, padding=1, mirror=mirror, zero_diag=True),
                       nn.ELU(inplace=True)])
