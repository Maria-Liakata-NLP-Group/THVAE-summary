from ..candle import utils, tracing
import iisignature
import torch
...
class path_sig_fn(autograd.Function):
    ...
    def forward(ctx, path, depth):
        ...
        path = path.detach().cpu().numpy()  # sloooow CPU :(
        ctx.path = path
        ctx.depth = depth
    ...
    def backward(ctx, grad_output):
        ...
        out = torch.tensor(backprop, dtype=torch.float, device=device)
...
batch_path_sig = utils.batch_fn(path_sig)
...
class Signature(nn.Module):
    ...
    def __init__(self, depth, **kwargs):
        if not isinstance(depth, tracing.Integer) or depth < 1:
