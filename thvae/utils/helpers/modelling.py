...
def group_att_over_input(inp_att_vals, inp_att_mask, att_indxs, att_indxs_mask,
    ...
    # group_num = att_indxs.size()[1]
    # att_vals = inp_att_vals[att_indxs][:, i, :, :].repeat(1, group_num, 1)
    ...
    # 这个inp_att_vals的值就是只抠出来一个post，按照mask抠，先抠第一个，然后第二个，然后第三个 。。。。。，在生成第一个post的时候，剩余post对它造成影响
    inp_att_vals = inp_att_vals[att_indxs] * \
                   att_indxs_mask.unsqueeze(-1).unsqueeze(-1)
    # inp_att_mask 也是这样啦，要生成的post为0， 其余位置为1
    inp_att_mask = inp_att_mask[att_indxs] * \
                   att_indxs_mask.unsqueeze(-1)
    ...
    if inp_att_keys is not None:
        # att_keys = inp_att_keys[att_indxs][:, i, :, :].repeat(1, group_num, 1)
...
def group_att_over_input_rev(inp_att_vals, inp_att_mask, att_indxs, att_indxs_mask,
                         inp_att_keys=None):
    """
    Returns properly concatenated and shaped attention keys/values/mask over
    input (e.g. hidden states and embds).
    ...
    att_vals = inp_att_vals.repeat(1, group_num, 1)
    inp_att_mask = inp_att_mask[att_indxs] * \
                   att_indxs_mask.unsqueeze(-1)
    ...
    if inp_att_keys is not None:
        ...
        #                att_indxs_mask.unsqueeze(-1).unsqueeze(-1)
        # att_keys = inp_att_keys.view(out_batch_size, total_els_per_unit, -1)
        att_keys = inp_att_keys.repeat(1, group_num, 1)
