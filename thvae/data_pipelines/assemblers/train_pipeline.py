...
from mltoolkit.mldp.steps.transformers.nlp.bart_transform import BartTransform
...
from thvae.data_pipelines.steps import GroupFileShuffler, SummRevIndxsCreator, \
    RevMapper
from thvae.utils.fields import InpDataF, ModelF
from csv import QUOTE_NONE


def assemble_train_pipeline(word_vocab, bart_tokenizer, max_groups_per_batch=1,
    ...
    tokenizer = TokenProcessor(fnames=[ModelF.REV, ModelF.PROMPT])
    # vocab_mapper = VocabMapper({ModelF.REV: word_vocab})

    # tokenizer_prom = TokenProcessor(fnames=ModelF.PROMPT)
    # vocab_mapper = VocabMapper({ModelF.REV: word_vocab, ModelF.PROMPT: word_vocab})

    bart_trans = BartTransform([ModelF.REV, ModelF.PROMPT], bart_tokenizer)
    # bart_trans = BartTransform([ModelF.REV], bart_tokenizer)



    # seq_wrapper = SeqWrapper(fname=[ModelF.REV, ModelF.PROMPT],
    ...
    seq_len_computer = SeqLenComputer([ModelF.REV, ModelF.PROMPT], [ModelF.REV_LEN, ModelF.PROMPT_LEN], bart_tokenizer)
    ...
    padder_prompt = Padder(fname=ModelF.PROMPT,
                        new_mask_fname=ModelF.PROMPT_MASK,
                        pad_symbol=word_vocab[PAD].id, padding_mode='right')
    ...
    pipeline.add_step(padder_prompt)
