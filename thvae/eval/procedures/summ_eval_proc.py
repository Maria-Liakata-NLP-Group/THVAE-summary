from thvae.utils.fields import ModelF, OutputF
from thvae.eval.metrics import GoogleRouge as Rouge
...
class SummEvalProc(object):
    ...
    def __init__(self, data_pipeline, bart_tokenizer, summs_gen_func, revs_formatter_func,
    ...
    def eval(self, data_source, output_file_path=None):
        ...
        for batch in self.data_pipeline.iter(**data_source):
            ...
            inp_revs = self.bart_tokenizer.batch_decode(batch[ModelF.REV], skip_special_tokens=True)
            # inp_revs = self.revs_formatter_func(torch.tensor(batch[ModelF.REV], dtype=torch.int))
